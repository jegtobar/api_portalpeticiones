...
class MetaMntoSatisfechosServiceImpl implements IMetasMntoSatisfechosService {
    ...
    function getMetas(int $id){
        $query = "SELECT a.id, 
        SUM(a.meta)AS meta, 
        (SELECT COUNT(b.id) FROM personas b WHERE b.seguimiento = 2 AND b.deleted_at IS NULL AND b.colonia_id = a.colonia_id) AS actual, 
        (SUM(a.meta)- (SELECT COUNT(b.id) FROM personas b WHERE b.seguimiento = 2 AND b.deleted_at IS NULL AND b.colonia_id = a.colonia_id))AS por_alcanzar,
        c.colonia,
